...
<div class="card">
    <div class="m-3 d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('Admin.CATCAdmin.training.create') }}" class="text-decoration-none">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="text-center align-top">Subjects</th>
                    ...
                    @else
                        @foreach($training as $item) <!-- Loop through each training item -->
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('Admin.CATCAdmin.participant.Detail',$item->id) }}">
                                        <i data-feather="eye"></i>
                                    </a>

                                    <a href="{{route('Admin.CATCAdmin.participant.create',$item->id)}}"><i data-feather="user-plus"></i></a>
                                </td>
                                <td class="text-center">
                                    ...
                                    <a href="{{route('Admin.CATCAdmin.training.costDetail',$item->id)}}">
                                        <i data-feather="eye"></i>
                                    </a>
                                </td>
                                <td class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{ route('Admin.CATCAdmin.training.edit', $item->id) }}" style="display: inline-block; vertical-align: middle;">
                                        <i data-feather="edit" class="text-primary"></i>
                                    </a>
                                    
                                    <form action="{{ route('Admin.CATCAdmin.training.Training.delete', $item->id) }}" method="POST" 
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @if (!empty($item)) 
                action="{{ route('Admin.CATCAdmin.training.update-status', $item->id) }}"
                @else 
                    action="#" 
                @endif >
                    @csrf
...
<div class="modal fade" id="uploadDocumentModal" tabindex="-1" aria-labelledby="uploadDocumentModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            @if(!empty($item)) 
            action="{{ route('Admin.CATCAdmin.training.documents.store',$item->id) }}"
            @else 
                action="#" 
            @endif 
            method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary"
                        @if(empty($item)) disabled @endif>Upload</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="subjectModal" tabindex="-1" aria-labelledby="subjectModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="subjectForm" action="{{ route('Admin.CATCAdmin.training.subject.store') }}" method="POST">
                @csrf
                <div class="modal-body">
        ...
        function initializeModals() {
            document.querySelectorAll(".open-cost-modal").forEach(button => {
                button.addEventListener("click", function () {
                    ...
                    if (costForm) {
                        costForm.action = `/Admin/CATCAdmin/training/cost-breakdown/store/${trainingId}`;
                        costModal.show();
                    }
                });
            });
            ...
            document.querySelectorAll(".open-status-modal").forEach(button => {
                button.addEventListener("click", function () {
                    const trainingId = this.dataset.trainingId;
                    document.getElementById("trainingStatusForm").action = `/Admin/CATCAdmin/training/update-status/${trainingId}`;
