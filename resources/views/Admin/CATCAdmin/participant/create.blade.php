...
<div class="card mt-2 ml-5 mr-5 mb-5 bg-gray-50 p-3 rounded-md shadow-lg">
    <div class="card-header d-flex justify-content-between">
        ...
        <button id="backButton" style="border: none; background: transparent; padding: 0;" type="button">
            <a href="{{ route('Admin.CATCAdmin.training.Detail') }}" class="text-white text-decoration-none">
    ...
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
        @unless(isset($participant))
            <!-- Search Form -->
            <form class="d-flex" method="GET" action="{{route('Admin.CATCAdmin.participant.create',$training->id ?? '')}}" style="max-width: 250px;">
                <input class="form-control me-2" type="search" name="epf_number" placeholder="Search here..." value="">
                <button class="btn btn-outline-success" type="submit">Search</button>
            </form>
        
            <!-- Download Button -->
            <a href="{{ route('Admin.CATCAdmin.participant.export-participant-columns') }}" class="btn btn-primary d-flex align-items-center px-3">
    ...
    <div class="card-body p-4 rounded-3 shadow-lg" style="background-color: #A8BDDB;">
        <form class="row g-3" action="{{ isset($participant) ? route('Admin.CATCAdmin.participant.update', $participant->id) : route('Admin.CATCAdmin.participant.store') }}" method="POST">
            @csrf
