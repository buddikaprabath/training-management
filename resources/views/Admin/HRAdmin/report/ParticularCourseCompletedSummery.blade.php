...
<div class="card card-custom">
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
        <h3 class="card-title">
            Particular Course Completed Summary
        </h3>
        <a href="{{ url()->current() }}">
        ...
        <!-- Download Button -->
        <a href="{{route('Admin.HRAdmin.report.pdf.dowload-Particular-Course-Completed-Summery-Pdf')}}" class="btn btn-primary d-flex align-items-center px-3">
    ...
    <div class="card-body">
        <form action="{{route('Admin.HRAdmin.report.ParticularCourseCompletedSummery')}}" method="GET">
            @csrf
    ...
    <div class="card-body p-4 rounded-3 shadow-lg" style="background-color: #A8BDDB;">
        @if($trainings->isNotEmpty())
            ...
            <div class="d-flex justify-content-between">
                <span>Course Name : {{ $firsttraining->training_name}}</span>
                <span>Year : {{ date('Y', strtotime($firsttraining->training_period_to)) }}</span>
            </div>
            <div class="d-flex justify-content-between">
                <span>Total Attendence : {{$attendedCount}}</span>
                <span>Training Days : {{$firsttraining->duration}}</span>
            </div>
            <div class="d-flex justify-content-between">
                <span>Period (From-to): {{$firsttraining->training_period_from}} - {{$firsttraining->training_period_to}}</span>
            </div>
        @else
        ...
        <table class="table table-hover table-checkable" id="kt_datatable">
            <thead>
                <tr>
                    ...
                    <th class="text-center align-top">Service N0.</th>
