...
<div class="card">
    <div class="m-3 d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('User.training.create') }}" class="text-decoration-none">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        @if (auth()->user()->division->division_name == 'CATC')
                        ...
                        @foreach($training as $item) <!-- Loop through each training item -->
                            <tr>
                                ...
                                <td class="text-center">
                                 ...
                                 <!-- Display Subject Modal for only catc users-->
                                 @if (auth()->user()->division->division_name == 'CATC')
                                    <td class="text-center">
                                 ...
                                 @endif
                                <td class="text-center">
                                    <a href="{{ route('User.participant.Detail',$item->id) }}">
                                        <i data-feather="eye"></i>
                                    </a>

                                    <a href="{{route('User.participant.create',$item->id)}}"><i data-feather="user-plus"></i></a>
                                </td>
                                <td class="text-center">
                                    ...
                                    <a href="{{route('User.training.costDetail',$item->id)}}">
                                        <i data-feather="eye"></i>
                                    </a>
                                </td>
                                <td class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{ route('User.training.edit', $item->id) }}" style="display: inline-block; vertical-align: middle;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="trainingStatusForm" method="POST" 
                    @if (!empty($item)) 
                        action="{{ route('User.training.update-status', $item->id) }}" 
                    @else 
                        action="#" 
                    @endif>
                    @csrf
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="documentUploadForm" 
                @if(!empty($item)) 
                    action="{{ route('User.training.documents.store', $item->id) }}" 
                @else 
                    action="#" 
                @endif 
...
<div class="modal fade" id="subjectModal" tabindex="-1" aria-labelledby="subjectModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="subjectForm" action="{{ route('User.training.subject.store') }}" method="POST">
                @csrf
                <div class="modal-body">
        ...
        function initializeModals() {
            document.querySelectorAll(".open-cost-modal").forEach(button => {
                button.addEventListener("click", function () {
                    ...
                    if (costForm) {
                        costForm.action = `/User/training/cost-breakdown/store/${trainingId}`;
                        costModal.show();
                    }
                });
            });
            ...
            document.querySelectorAll(".open-status-modal").forEach(button => {
                button.addEventListener("click", function () {
                    const trainingId = this.dataset.trainingId;
                    document.getElementById("trainingStatusForm").action = `/User/training/update-status/${trainingId}`;
